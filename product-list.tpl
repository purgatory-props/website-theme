{if !empty($products)}
    <ul {if !empty($id)}id="{$id}"{/if} class="product_list grid list-grid row{if !empty($class)} {$class}{/if}">
        ...
        {foreach from=$products item=product}<li class="product-list-item grid-item ajax_block_product {if isset($product_block_size_class)}{$product_block_size_class}{/if}">
