<button id="createGuestBtn" class="btn btn-success" style="display: block; margin: 0 auto; width: 100%">{l s='Don\'t Want to Make an Account? Click Here to Continue as a Guest'}</button>
<button id="createAccountBtn" class="btn btn-default" style="display: none; margin: 0 auto; width: 100%">{l s='Create an Account Instead'}</button>

<form action="{$link->getPageLink('authentication', true, NULL, "back=$back")|escape:'html':'UTF-8'}" method="post" id="new_account_form" class="std clearfix" style="display: none">
  <div class="">
    <div id="opc_account_form" style="display: block; ">
      <h3 class="page-heading">{l s='Guest Checkout'}</h3>
      <p class="required"><sup class="required">*</sup>{l s='Required Field'}</p>
      <div class="required form-group">
        <label for="guest_email">{l s='Email'} <sup class="required">*</sup></label>
        <input type="text" class="is_required validate form-control" data-validate="isEmail" id="guest_email" name="guest_email" value="{if isset($smarty.post.guest_email)}{$smarty.post.guest_email}{/if}" autocomplete="username">
      </div>

      <div class="required form-group">
        <label for="firstname">{l s='First Name'} <sup class="required">*</sup></label>
        <input type="text" class="is_required validate form-control" data-validate="isName" id="firstname" name="firstname" value="{if isset($smarty.post.firstname)}{$smarty.post.firstname}{/if}" autocomplete="given-name">
      </div>
      <div class="required form-group">
        <label for="lastname">{l s='Last Name'} <sup class="required">*</sup></label>
        <input type="text" class="is_required validate form-control" data-validate="isName" id="lastname" name="lastname" value="{if isset($smarty.post.lastname)}{$smarty.post.lastname}{/if}" autocomplete="family-name">
      </div>
      ...
      {if isset($optin) && $optin}
        <div class="checkbox">
          <label for="optin">
            ...
            <span class="label-text">
            {l s='Receive Special Offers'}
            </span>
          </label>
        </div>
      {/if}
      <h3 class="page-heading">{l s='Shipping Address'}</h3>
      {foreach from=$dlv_all_fields item=field_name}
        ...
        {elseif $field_name eq "vat_number"}
          ...
          <div id="vat_number" style="display:none;">
            <div class="form-group">
              <label for="vat-number">{l s='VAT Number'}{if in_array($field_name, $required_fields)} <sup class="required">*</sup>{/if}</label>
        ...
        {elseif $field_name eq "address1"}
          <div class="required form-group">
            ...
            <input type="text" class="form-control" name="address1" id="address1" value="{if isset($smarty.post.address1)}{$smarty.post.address1}{/if}" autocomplete="address-line1">
          </div>
        {elseif $field_name eq "address2"}
          <div class="form-group is_customer_param">
            <label for="address2">{l s='Address (Line 2)'}{if in_array($field_name, $required_fields)} <sup class="required">*</sup>{/if}</label>
            <input type="text" class="form-control" name="address2" id="address2" value="{if isset($smarty.post.address2)}{$smarty.post.address2}{/if}" autocomplete="address-line2">
          </div>
        ...
        {elseif $field_name eq "city"}
          <div class="required form-group">
            ...
            <input type="text" class="form-control" name="city" id="city" value="{if isset($smarty.post.city)}{$smarty.post.city}{/if}" autocomplete="address-level2">
          </div>
          {* if customer hasn't update his layout address, country has to be verified but it's deprecated *}
        {elseif $field_name eq "Country:name" || $field_name eq "country"}
          <div class="required select form-group">
            <label for="id_country">{l s='Country'} <sup class="required">*</sup></label>
            <select name="id_country" id="id_country" class="form-control">
              {foreach from=$countries item=v}
                <option value="{$v.id_country}"{if (isset($smarty.post.id_country) AND  $smarty.post.id_country == $v.id_country) OR (!isset($smarty.post.id_country) && $sl_country == $v.id_country)} selected="selected"{/if} autocomplete="country">{$v.name}</option>
              {/foreach}
            </select>
          </div>
        {elseif $field_name eq "State:name"}
          ...
          <div class="required id_state select form-group">
            <label for="id_state">{l s='State'} <sup class="required">*</sup></label>
            <select name="id_state" id="id_state" class="form-control" autocomplete="address-level1">
              <option value="">-</option>
            </select>
          </div>
        {/if}
      ...
      <input type="hidden" name="alias" id="alias" value="{l s='Your Address'}">
      ...
      <div class="checkbox">
        <label for="invoice_address">
          ...
          <span class="label-text">
          {l s='Use Different Billing Address'}
          </span>
        </label>
      </div>
      <div id="opc_invoice_address"  class="unvisible">
        ...
        {foreach from=$inv_all_fields item=field_name}
          {if $field_name eq "company"}
            <div class="form-group">
              <label for="company_invoice">{l s='Company'}{if in_array($field_name, $required_fields)} <sup class="required">*</sup>{/if}</label>
          ...
          {elseif $field_name eq "vat_number"}
            ...
            <div id="vat_number_block_invoice" style="display:none;">
              <div class="form-group">
                <label for="vat_number_invoice">{l s='VAT Number'}{if in_array($field_name, $required_fields)} <sup class="required">*</sup>{/if}</label>
          ...
          {elseif $field_name eq "dni"}
            ...
            <div class="required form-group dni_invoice">
              <label for="dni_invoice">{l s='Identification Number'} <sup class="required">*</sup></label>
          ...
          {elseif $field_name eq "firstname"}
            <div class="required form-group">
              <label for="firstname_invoice">{l s='First Name'} <sup class="required">*</sup></label>
          ...
          {elseif $field_name eq "lastname"}
            <div class="required form-group">
              <label for="lastname_invoice">{l s='Last Name'} <sup class="required">*</sup></label>
          ...
          {elseif $field_name eq "address1"}
            <div class="required form-group">
              <label for="address1_invoice">{l s='Address'} <sup class="required">*</sup></label>
          ...
          {elseif $field_name eq "address2"}
            <div class="form-group is_customer_param">
              <label for="address2_invoice">{l s='Address (Line 2)'}{if in_array($field_name, $required_fields)} <sup class="required">*</sup>{/if}</label>
          ...
          {elseif $field_name eq "postcode"}
            ...
            <div class="required postcode_invoice form-group">
              <label for="postcode_invoice">{l s='Zip/Postal Code'} <sup class="required">*</sup></label>
          ...
          {elseif $field_name eq "city"}
            <div class="required form-group">
              <label for="city_invoice">{l s='City'} <sup class="required">*</sup></label>
          ...
          {elseif $field_name eq "country" || $field_name eq "Country:name"}
            <div class="required form-group">
              <label for="id_country_invoice">{l s='Country'} <sup class="required">*</sup></label>
          ...
          {elseif $field_name eq "state" || $field_name eq 'State:name'}
            ...
            <div class="required id_state_invoice form-group" style="display:none;">
              <label for="id_state_invoice">{l s='State'} <sup class="required">*</sup></label>
