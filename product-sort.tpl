{if isset($orderby) AND isset($orderway)}
  ...
  <div id="productsSortForm{if isset($paginationId)}_{$paginationId}{/if}" class="form-group productsSortForm">
    <form action="{$request|escape:'html':'UTF-8'}">
      <label for="selectProductSort{if isset($paginationId)}_{$paginationId}{/if}" class="small">{l s='Sort By'}</label>
      <select id="selectProductSort{if isset($paginationId)}_{$paginationId}{/if}" class="selectProductSort form-control">
        ...
        {if !$PS_CATALOG_MODE}
          <option value="price:asc"{if $orderby eq 'price' AND $orderway eq 'asc'} selected="selected"{/if}>{l s='Price: Lowest'}</option>
          <option value="price:desc"{if $orderby eq 'price' AND $orderway eq 'desc'} selected="selected"{/if}>{l s='Price: Highest'}</option>
        {/if}
        ...
        {/if}
        <option value="reference:asc"{if $orderby eq 'reference' AND $orderway eq 'asc'} selected="selected"{/if}>{l s='Model: Lowest'}</option>
        <option value="reference:desc"{if $orderby eq 'reference' AND $orderway eq 'desc'} selected="selected"{/if}>{l s='Model: Highest'}</option>
      </select>
    </form>
  </div>
